...
def path_includes_gemfile?(path)
  dir_parts = File.dirname(path).split("/")
  current_path = "/"
  dir_parts.each do |part|
    current_path = File.join(current_path, part)
    return true if File.exists?(File.join(current_path, "Gemfile"))
  end
  false
end
...
if ENV['TM_BUNDLE'] || ENV['TM_RUBY'].to_s =~ /\.rvm/
  bundle_cmd = if ENV['TM_BUNDLE']
    ENV['TM_BUNDLE']
  elsif path_includes_gemfile?(ENV['TM_FILEPATH'])
  ...
  if bundle_cmd and File.exists?(bundle_cmd)

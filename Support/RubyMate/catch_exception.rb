...
at_exit do
  if (e = $!) && !e.instance_of?(SystemExit)
    ...
    io.write "<p id='exception'><strong>#{e.class.name}:</strong> #{htmlize e.message.sub(/`(\w+)'/, '‘\1’').sub(/ -- /, ' — ')}</p>\n"
    ...
    e.backtrace.each do |b|
      if b =~ /(.*?):(\d+)(?::in\s*`(.*?)')?/ then
        ...
        io << (method ? "method #{htmlize method}" : '<em>at top level</em>')
        io << "</a></td>\n<td>in <strong>#{htmlize display_name}</strong> at line #{line}</td></tr>\n"
      end
    end
